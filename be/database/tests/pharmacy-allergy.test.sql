...
BEGIN TRY
    ...
    DECLARE @medicine_public_id uniqueidentifier,@prescription_public_id uniqueidentifier,
      @item_public_id uniqueidentifier,@medicine_code varchar(30)=CONCAT('PA',LEFT(@suffix,20));
    INSERT dbo.patient_allergies(patient_id,allergen_name,allergy_type,severity,recorded_by_user_id)
    VALUES(@patient_id,N'amoxicillin','DRUG','SEVERE',@admin_id);
    EXEC sys.sp_set_session_context @key=N'actor_user_id',@value=@admin_id;
    ...
    EXEC dbo.sp_clinic_add_prescription_item @actor_user_id=@doctor_user_id,
      ...
      @prescribed_quantity=2,@dose=N'1 viên',@frequency=N'Ngày hai lần',
      @usage_instruction=N'Uống sau ăn',
      @allergy_override_reason=N'Đã đánh giá dị ứng và cân nhắc lợi ích nguy cơ',
      @item_public_id=@item_public_id OUTPUT;
