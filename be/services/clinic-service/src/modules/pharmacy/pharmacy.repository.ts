...
export class SqlPharmacyRepository implements PharmacyRepository {
  ...
  async get(actor: ClinicPrincipal, publicId: string, requestId: string) {
    ...
    return {
      ...
      branch: { publicId: id(r.branchPublicId), code: id(r.branchCode), name: id(r.branchName),
        timezoneName: id(r.timezoneName), medicalLicenseNo: str(r.branchMedicalLicenseNo),
        phone: str(r.branchPhone), email: str(r.branchEmail), addressLine: id(r.branchAddressLine),
        ward: str(r.branchWard), district: str(r.branchDistrict), province: str(r.branchProvince) },
      prescriber: { publicId: id(r.prescriberPublicId), fullName: id(r.prescriberFullName),
        medicalLicenseNo: id(r.prescriberMedicalLicenseNo), academicTitle: str(r.prescriberAcademicTitle) },
      patient: { dateOfBirth: date(r.patientDateOfBirth), gender: id(r.patientGender),
        phone: str(r.patientPhone), addressLine: str(r.patientAddressLine),
        healthInsuranceNo: str(r.patientHealthInsuranceNo) },
      clinicalNotes: str(r.clinicalNotes), generalInstructions: str(r.generalInstructions),
