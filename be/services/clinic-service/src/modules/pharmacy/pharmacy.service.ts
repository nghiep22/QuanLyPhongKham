...
export class PharmacyService {
  ...
  async run<T>(operation: () => Promise<T>): Promise<T> {
    try { return await operation(); } catch (error) {
      ...
      if ([2601, 2627, 53910, 53912, 53917, 53919, 53920, 53302, 53304, 53307, 53308, 53309, 53313,
        ...
        53341, 53342, 53344, 53345, 53347, 53348, 53922, 52045, 52046, 52047, 52048,
        52049, 52063, 52064].includes(code ?? 0))
        throw new HttpError(409, 'PHARMACY_CONFLICT', code === 53910
          ? 'Thuốc trùng với dị ứng đã ghi nhận. Cần quyền override và lý do.'
      ...
      if ([53904, 53906, 53909, 53918, 53921, 53300, 53305, 53310, 53311, 53316, 53324,
        53337, 53338, 53343, 53346].includes(code ?? 0))
