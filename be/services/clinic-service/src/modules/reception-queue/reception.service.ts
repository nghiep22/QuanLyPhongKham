...
function mapError(error: unknown): never {
  ...
  if ([53750, 53752, 53753].includes(code ?? 0)) throw new HttpError(404, 'RECEPTION_RESOURCE_NOT_FOUND', 'Không tìm thấy dữ liệu tiếp nhận được yêu cầu.');
  ...
  if ([53201, 53751].includes(code ?? 0)) throw new HttpError(400, 'RECEPTION_VALIDATION_ERROR', 'Thông tin tiếp nhận không hợp lệ.');
  throw error;
}
